...
export default function LoginPage() {
  ...
  async function sendLink(event:FormEvent){event.preventDefault(); const supabase=createClient(); const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:`${location.origin}/auth/callback`}}); setMessage(error?error.message:'Check your email for a secure sign-in link.');}
